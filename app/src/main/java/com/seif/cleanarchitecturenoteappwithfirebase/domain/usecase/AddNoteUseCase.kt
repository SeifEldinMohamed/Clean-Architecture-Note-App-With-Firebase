    ...
    operator fun invoke(note: Note) = flow {
        when (val result = validNote(note)){
           is Resource.Success -> emit(result.data)
            is Resource.Error -> emit(result.message)
        }

    }

    private fun validNote(note: Note): Resource<Flow<Resource<String, String>>, String> {
        return if (note.title.length < 5){
            ...
            Resource.Success(noteRepository.addNote(note))
        }
    }
}